<template>
  <div class="container flip-clock">
    ...
    >
      ...
      >
        <span v-if="data.label !== 'Hours' && data.label !== 'Days'" class="flip-clock__card flip-card">
        ...
        <span v-if="data.label !== 'Hours' && data.label !== 'Days'" class="flip-clock__slot">{{ data.label }}</span>
      </span>
    </template>
  </div>
</template>
    ...
    timerFinished: {
      handler (newVal, oldVal) {
        if (newVal) {
          ...
          console.log('bitch nigga')
          this.$store.commit('timer/changeTimeIsDone')
        }
      }
    }
  },
